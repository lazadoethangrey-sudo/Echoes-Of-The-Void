...
const SAVE_KEY = 'ECHOES_VOID_PERMANENT_V10';
const LATEST_SAVE_VERSION = 10;

// Improved Deep Merge: Recursively merges objects while preserving arrays from source
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    gameState.shards, 
    gameState.heroTickets, 
    gameState.itemTickets, 
    gameState.totalAccountExp, 
    gameState.unlockedStages, 
    gameState.party, 
    gameState.allHeroes,
    gameState.inventory,
    gameState.settings, 
    gameState.totalDailyClaims,
    gameState.lastDailyClaim
  ]);
  ...
  const selectStage = (stage: Stage) => {
    ...
    getStageDialogueStream(stage.name, stage.description, stage.loreNote, (chunk) => {
      ...
      setGameState(prev => ({ 
        ...prev, 
        storyDialogue: [{ speaker: "NARRATOR", text: stage.loreNote }, ...lines] 
      }));
    }).finally(() => {
  ...
  const onVictory = () => {
    ...
    const updatedParty = gameState.party.map(hero => {
      ...
      while (h.exp >= h.maxExp) {
        h.exp -= h.maxExp;
        h.level += 1;
        h.maxExp = Math.floor(h.maxExp * 1.2);
        h.maxHp += 8; 
        h.hp = h.maxHp;
        h.attack += 2;
        h.defense += 1;
    ...
    setGameState(prev => ({ 
      ...prev, 
      party: updatedParty,
      allHeroes: newHeroList,
      shards: prev.shards + shardsWon,
      heroTickets: newHeroTickets,
      itemTickets: newItemTickets,
      totalAccountExp: prev.totalAccountExp + expWon,
  ...
  return (
    <div className="w-full h-screen bg-black text-white font-sans overflow-hidden flex flex-col">
      <div 
        ref={cursorRef} 
        className="virtual-cursor flex items-center justify-center" 
        style={{ position: 'fixed', top: 0, left: 0, transform: 'translate3d(-100px, -100px, 0)' }}
      >
        <div className="relative w-8 h-8 flex items-center justify-center">
            ...
            <div className="absolute w-8 h-[1px] bg-violet-500/40"></div>
            <div className="absolute w-[1px] h-8 bg-violet-500/40"></div>
      ...
      <div className="flex-1 relative overflow-hidden">
        {gameState.screen === GameScreen.LORE_INTRO && (
          <LoreIntro onComplete={() => navigateTo(GameScreen.TITLE)} />
        )}
        ...
        {gameState.screen === GameScreen.MAP && (
          <div className="relative h-full">
            <StageSelect 
              ...
              onBackToTitle={() => navigateTo(GameScreen.TITLE)}
            ...
            <DailyReward 
              claimed={!showDaily} 
              onClaim={claimDaily} 
              totalClaims={gameState.totalDailyClaims} 
            />
          </div>
        )}
        ...
        {gameState.screen === GameScreen.BATTLE && gameState.currentStage && (
          <BattleScene 
            ...
            waves={gameState.currentStage.waves} 
            onVictory={onVictory}
            onDefeat={() => { soundService.play('DEFEAT'); navigateTo(GameScreen.DEFEAT); }}
            onRetreat={() => navigateTo(GameScreen.MAP)}
            stageName={gameState.currentStage.name}
            shards={gameState.shards}
            settings={gameState.settings}
          />
        )}
        ...
        {gameState.screen === GameScreen.GACHA && (
          <GachaSystem 
            shards={gameState.shards} 
            heroTickets={gameState.heroTickets}
            itemTickets={gameState.itemTickets}
            onAddTickets={(types) => setGameState(p => {
              const cost = types.length * 100;
              if (p.shards < cost) return p;
              let newHero = p.heroTickets;
              let newItem = p.itemTickets;
              types.forEach(t => {
                if (t === 'ECHO') newHero++;
                else newItem++;
              });
            ...
            onConsumeTickets={(type, count) => setGameState(p => ({
                ...p,
                heroTickets: type === 'CHARACTER' ? p.heroTickets - count : p.heroTickets,
                itemTickets: type === 'ITEM' ? p.itemTickets - count : p.itemTickets,
            }))}
        ...
        {gameState.screen === GameScreen.VICTORY && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-2xl p-4 animate-in fade-in duration-700">
            <div className="max-w-2xl w-full flex flex-col items-center">
               ...
               {showRewards?.newHero && (
                  <div className="mb-12 flex flex-col items-center animate-bounce">
                     <div className="text-amber-400 font-black font-cinzel text-xs tracking-widest uppercase mb-4">New Echo Resonated!</div>
                     <div className="text-3xl font-cinzel text-white font-bold">{showRewards.newHero}</div>
                  </div>
               )}

               <button 
                  onClick={() => navigateTo(GameScreen.MAP)}
                  className="px-24 py-6 bg-white text-slate-950 font-black font-cinzel tracking-widest rounded-[2rem] hover:bg-emerald-500 hover:text-white transition-all active:scale-95 shadow-2xl text-xl uppercase"
               >
                  Return to Archive
               </button>
            </div>
        ...
        {gameState.screen === GameScreen.DEFEAT && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-red-950/20 backdrop-blur-3xl p-4 animate-in fade-in duration-1000">
            <div className="max-w-xl w-full flex flex-col items-center text-center">
               <div className="w-24 h-24 bg-red-600/20 rounded-full flex items-center justify-center mb-8 border border-red-500/30 text-red-500 text-4xl animate-pulse">
               ...
               <button 
                  onClick={() => navigateTo(GameScreen.MAP)}
                  className="px-20 py-5 bg-red-600 hover:bg-red-500 text-white font-black font-cinzel tracking-widest rounded-2xl transition-all active:scale-95 shadow-xl text-lg uppercase"
               >
